    ...
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className='fixed bottom-5 right-5 z-50 w-[300px] max-sm:bottom-0 max-sm:left-0 max-sm:right-0 max-sm:w-full'
        >
          {/* Close button */}
          <button
            onPointerDown={(e) => { e.preventDefault(); e.stopPropagation(); dismiss(); }}
            className='absolute right-3 top-3 z-10 flex h-6 w-6 items-center justify-center rounded-full border border-white/10 bg-theme-700/60 text-neutral-500 transition-colors hover:border-white/20 hover:text-white'
            aria-label={t('close')}
          >
            <X className='h-3 w-3' />
          </button>
          ...
          <a
            ...
            className='group relative flex flex-col gap-3 overflow-hidden rounded-2xl border border-white/10 bg-theme-800/95 p-5 shadow-2xl shadow-black/60 backdrop-blur-xl transition-colors hover:border-theme-400/30 max-sm:rounded-b-none max-sm:border-b-0'
          >
            {/* Glow */}
            <div
              aria-hidden
            ...
            <p className='text-xs leading-relaxed text-neutral-400'>
              {t('body')}{' '}
              <span className='font-semibold text-neutral-300'>{t('body_highlight')}</span>{' '}
              {t('body_suffix')}
            </p>
            ...
            <div className='flex items-center justify-between rounded-xl bg-theme-400/10 border border-theme-400/20 px-3.5 py-2.5 transition-colors group-hover:bg-theme-400/15'>
              <span className='text-xs font-bold text-theme-400'>{t('cta')}</span>
